...
routerApp.controller('cloth_colorCTRL', function($localStorage,$window,$cookies, $rootScope,$scope, $http, $state,Pagination,service) {
      ...
      .success(function(data) {
        if (data.errors) {
          ...
          for(var i= 0; i < data.length; i++) {
            if($rootScope.aa[data[i].color] == null)
              $rootScope.aa[data[i].color] = 1;
            else
              $rootScope.aa[data[i].color]++
          }
          var value = [];
      ...
      .success(function(da) {
        if (da.errors) {
          ...
          for(var j = 0; j < da.length; j++) {
            $rootScope.arr[j] = da[j].color_name;
          }
