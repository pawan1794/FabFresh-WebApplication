...
routerApp.controller('cloth_sizeCTRL', function($localStorage,$window,$cookies, $rootScope,$scope, $http, $state,Pagination,service) {
      ...
      .success(function(data) {
        if (data.errors) {
          ...
          for(var i= 0; i < data.length; i++) {
            if($rootScope.aa[data[i].size] == null)
              $rootScope.aa[data[i].size] = 1;
            else
              $rootScope.aa[data[i].size]++
          }
          var value = [];
      ...
      .success(function(da) {
        if (da.errors) {
          ...
          for(var j = 0; j < da.length; j++) {
            $rootScope.arr[j] = da[j].size_name;
          }
